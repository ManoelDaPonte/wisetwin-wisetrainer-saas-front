...
export function useUnityEvents(courseId = null) {
    ...
    async (event) => {
      ...
      try {
        ...
        if (showGuide && currentGuide && data.name) {
          console.log(`🔍 Vérification de validation de guide: ${data.name}`);
        ...
        if (data.name) {
          ...
          if (!scenarioId) {
            if (
              ...
              else if (workerNum === "2") scenarioId = "smoking-worker";
              else if (workerNum === "3") scenarioId = "chemical-hazard";
          ...
          if (scenarioId) {
            ...
            console.warn(`❌ Aucun scénario trouvé pour l'objet ${data.name}`);
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("QuestionnaireRequest", handleQuestionnaireRequest);
    window.addEventListener("GuideValidationEvent", handleGuideValidation);

    // Événements supplémentaires pour le débogage
    window.addEventListener("ObjectNamesReceived", handleObjectNamesReceived);
    ...
    window.addEventListener("DropdownDataReceived", handleDropdownDataReceived);
    ...
    return () => {
      ...
      window.removeEventListener("GuideValidationEvent", handleGuideValidation);
      ...
      window.removeEventListener("GUIDDataReceived", handleGUIDDataReceived);
      window.removeEventListener(
        "SphereDataReceived",
