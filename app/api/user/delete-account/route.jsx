...
export async function DELETE() {
  try {
    ...
    if (process.env.AUTH0_MANAGEMENT_CLIENT_ID && process.env.AUTH0_MANAGEMENT_CLIENT_SECRET) {
      ...
      console.warn("Identifiants de gestion Auth0 non disponibles - l'utilisateur n'a pas été supprimé d'Auth0");
    }
...
async function deleteAzureContainer(containerName) {
  try {
    ...
    if (exists) {
      ...
      console.log(`Container ${containerName} non trouvé, aucune suppression nécessaire`);
    }
  } catch (error) {
...
async function deleteAuth0User(userId) {
  try {
    ...
    console.error(`Erreur lors de la suppression de l'utilisateur Auth0 ${userId}:`, error);
    throw error;
  }
}