...
import { useActiveContext, useContextCourses, usePermissions } from "@/newlib/hooks";
...
export default function WiseTrainerCourses() {
  ...
  const handleEnroll = async (course) => {
    try {
      ...
      if (result.success) {
        ...
        const compositeId = course.compositeId || 
          `${course.id}__${activeContext.type}__${isPersonalMode ? user?.id : currentOrganization?.id}`;
  ...
  const confirmUnenroll = async () => {
    ...
    try {
      ...
      toast({
        title: "Erreur",
        description: error.message || "Impossible de se désinscrire de la formation",
        variant: "destructive",
      });
    }
  };
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            ...
            <TabsTrigger value="organization">
              {isPersonalMode ? "Catalogue" : "Formations disponibles"} ({catalogCourses.length})
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="organization" className="mt-6">
            <CatalogOrganizationTab
              ...
              selectedOrganization={isOrganizationMode ? currentOrganization : null}
              selectedOrganizationId={isOrganizationMode ? currentOrganization?.id : null}
              flippedCardId={flippedCardId}
              setFlippedCardId={setFlippedCardId}
              containerVariants={containerVariants}
              itemVariants={itemVariants}
