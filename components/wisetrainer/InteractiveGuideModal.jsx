...
export default function InteractiveGuideModal({
  ...
  }, [currentStep, completedSteps, currentStepIndex, guide.steps, onComplete]);
  ...
  useEffect(() => {
    const handleValidationEvent = (event) => {
      ...
      if (tutorialStarted && currentStep) {
        ...
        if (currentStep.validationType === "checkbox") {
          console.log("⏭️ Étape de type checkbox, ignorer événement Unity");
          return;
        }
        ...
        if (
          ...
          // Vérifier si l'objet cliqué fait partie de la liste des objets du guide
          const isObjectInSequence =
            Array.isArray(guide.sequenceButtons) &&
            guide.sequenceButtons.includes(buttonName);

          if (isObjectInSequence) {
            // CAS 1: Objet dans la liste mais dans le mauvais ordre → RESET COMPLET
            console.log(
              "🔄 Objet dans la séquence mais mauvais ordre - Reset du guide"
            );

            // RESET INSTANTANÉ du guide
            setCurrentStepIndex(0);
            setCompletedSteps([]);
            setShowHint(false);

            // Afficher un feedback d'erreur spécifique pour mauvais ordre
            setErrorFeedback(true);
            setLastErrorMessage(
              `Séquence incorrecte ! Vous avez cliqué sur "${buttonName}" mais l'étape actuelle demande "${
            ...
            // CAS 2: Objet pas dans la liste → SIMPLE AVERTISSEMENT (pas de reset)
            console.log(
              "⚠️ Objet non pertinent pour la procédure - Simple avertissement"
            );

            // Juste afficher un feedback d'erreur temporaire
            setErrorFeedback(true);
            setLastErrorMessage(
              `L'objet "${buttonName}" ne fait pas partie de cette procédure. Veuillez sélectionner un élément approprié.`
            );
    ...
    const handleGameObjectSelected = (event) => {
      ...
      if (data && data.name && tutorialStarted) {
        ...
        console.log(`🔍 GameObject sélectionné dans le guide: ${data.name}`);

        // Créer et dispatcher un événement de validation
        const validationEvent = new CustomEvent("GuideValidationEvent", {
          detail: {
            name: data.name,
    ...
    return () => {
      window.removeEventListener("GuideValidationEvent", handleValidationEvent);
  ...
  const simulateValidation = () => {
    if (!tutorialStarted) {
      ...
      if (onStartTutorial) {
        ...
        console.warn("La fonction onStartTutorial n'est pas disponible");
      }
    } else {
      ...
      <div className="fixed z-40 top-16 right-0 bottom-4 w-2/5 max-w-xl bg-white/95 dark:bg-gray-800/95 rounded-lg shadow-xl flex flex-col border border-gray-200 dark:border-gray-700">
        ...
        <div className="p-6 flex-1 overflow-y-auto">
          <div className="mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
            ...
            <div className="prose prose-sm dark:prose-invert max-w-none">
              ...
              {educationalContent.content.intro && (
                <p className="mb-3">{educationalContent.content.intro}</p>
              )}

              {/* Affichage des sections */}
              {educationalContent.content.sections &&
                educationalContent.content.sections.map((section, index) => (
          ...
          <div className="text-center">
            <h3 className="text-xl font-bold mb-4">Tutoriel interactif</h3>
            <p className="mb-6">
              {guide.startMessage ||
    ...
    <div className="fixed z-40 top-16 right-0 bottom-4 w-2/5 max-w-xl bg-white/95 dark:bg-gray-800/95 rounded-lg shadow-xl flex flex-col border border-gray-200 dark:border-gray-700">
      ...
      <div className="p-6 flex-1 overflow-y-auto">
        <div className="mb-4">
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-gray-800 dark:text-gray-200">
                    {lastErrorMessage || "Veuillez essayer à nouveau."}
                  </p>
          ...
          {isCheckboxStep && !completedSteps.includes(currentStep.id) && (
            ...
            <div className="mt-2 bg-gray-50 dark:bg-gray-700 p-2 rounded-md">
              ...
              <div className="flex items-center">
                <div className="bg-blue-500 text-white w-6 h-6 flex items-center justify-center rounded-full mr-2 pulse-animation">
                  <span className="text-xs">{currentStepIndex + 1}</span>
                </div>
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400 mt-1 ml-8">
                {completedSteps.length} sur {guide.steps?.length} étapes
                complétées
              </div>
            </div>
          )}
