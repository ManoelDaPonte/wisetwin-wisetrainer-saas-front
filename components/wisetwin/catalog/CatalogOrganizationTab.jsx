...
export default function CatalogOrganizationTab({
  ...
  // Enrichir les builds avec les métadonnées
  const enrichedBuilds = builds.map((build) => {
    const enrichedBuild = enrichBuildWithMetadata({
      ...
      source: {
        type: "organization",
        name: selectedOrganization ? selectedOrganization.name : "Organisation",
      },
    });
    ...
    if (showNoOrganizations) {
      return (
        <EmptyStateCard
          ...
          description="Vous n'êtes membre d'aucune organisation. Rejoignez une organisation pour accéder aux environnements 3D."
        />
      );
    }
    ...
    <div className="space-y-6">
      ...
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-wisetwin-darkblue dark:text-white">
          Catalogue d'environnements 3D
        </h2>
        <p className="text-gray-600 dark:text-gray-300">
          Explorez les environnements 3D proposés par vos organisations
        </p>
      </div>
